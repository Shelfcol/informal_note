...
End-to-end SLAM almost abondons the traditional SLAM pipeline,especially the geometry application. [DeepVO] utilized RCNN network to obtain camera ego-motion. [Unsupervised Learning of Depth and Ego-Motion from Video] explored an unsupervised learning framework to estimate monocular depth and camera motion. They just used video data to train their network,which can be heful to network training.
