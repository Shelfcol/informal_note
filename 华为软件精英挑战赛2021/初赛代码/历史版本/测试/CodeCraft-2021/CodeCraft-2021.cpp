...
// typedef twoDArr[MAXCORE][MAXMEM];
// vector<twoDArr> pathBag;
...
void proceessVmPoolBetweenTwoDels() {
    ...
    // sort(vmsOfSingleNode.begin(), vmsOfSingleNode.end(), [&](vmEntity * a, vmEntity * b) {
    //     return a->core + a->memory > b->core + b->memory;
    // });

    // sort(vmsOfDoubleNode.begin(), vmsOfDoubleNode.end(), [&](vmEntity * a, vmEntity * b) {
    //     return a->core + a->memory > b->core + b->memory;
    // });
    ...
    //         break;
    //     if(server->A.core > 50 && server->A.memory > 50)
    //         solveBagProblem(vmsOfSingleNode, it->second, type_A);
    //     if(vmsOfSingleNode.size() < 0)
    //         break;
    //     if(server->B.core > 50 && server->B.memory > 50)
    //         solveBagProblem(vmsOfSingleNode, it->second, type_B);
    // }
    ...
    //         break;
    //     if(server->A.core > 50 && server->A.memory > 50)
    //         solveBagProblem(vmsOfDoubleNode, it->second, type_D);
    // }
...
void proceessVmPoolThisBatchNotProccessed() {
    ...
    //处理单节点服务器
    while((int)vmsOfSingleNode.size() > 0) {
        ...
        solveBagProblem(vmsOfSingleNode, newServer, type_A);///////////////dead
        if(vmsOfSingleNode.size() > 0) {
    ...
    // //处理单节点服务器
    // for(vmEntity* vm: vmsOfSingleNode) {
    //     if(!firstFit(vm)) {
    //         serverEntity* newServer = serverInfos[serverType2Info[serverNameToBeBought]].clone(serverNumber);//购买一个新的服务器
    //         serverFor1Node[serverNumber] = newServer;
    //         bindVm2Server(vm, newServer, type_A);
    //         outManager.addOutputInfo(newServer->serverId, type_A, vm->time.day, vm->time.idx);
    //         ++serverNumber;
    //     }
    // }
    // //处理双节点服务器
    // for(vmEntity* vm: vmsOfDoubleNode) {
    //     if(!firstFit(vm)) {
    //         serverEntity* newServer = serverInfos[serverType2Info[serverNameToBeBought]].clone(serverNumber);//购买一个新的服务器
    //         serverFor1Node[serverNumber] = newServer;
    //         bindVm2Server(vm, newServer, type_D);
    //         outManager.addOutputInfo(newServer->serverId, type_D, vm->time.day, vm->time.idx);
    //         ++serverNumber;
    //     }
    // }
...
void processOneDay(vector<requestInfo>& oneDayRequests, int day) {
    ...
    for (int i = 0; i < oneDayRequests.size(); ++i) {
        while(i < oneDayRequests.size() && oneDayRequests[i].type == DEL) {
            ...
            if(serverResourceRecycleOfThisVm(vmID)) {//回收vmID部署的服务器的资源
...
void processOneBatch(int dayBegin) { //处理一个批次的请求
    if(vmPoolThisBatchNotProccessed.vmPool.size() != 0) {
    ...
    for(auto& reqs : toBeProcessedRequests) {
        processOneDay(reqs, day);
        ++day;
    }

    //一个batch完了之后处理vmPoolThisBatchNotProccessed的虚拟机
    // cout << "count: " << count << "\n";
    proceessVmPoolThisBatchNotProccessed();
    // cout << "count: " << count << "\n";
...
int main() {
    ...
    //input_test();
    int dayNumOfOneBatch = 4;//每次处理20天的
