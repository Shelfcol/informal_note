...
double ratioCoreMemSplit[typesOfServer2Choose + 1] = {0.15, 0.75, 1.2, 6};
double coreMemCapSplit[2][typesOfServer2Choose + 1];
...
int getRowOfvm2server(vmEntity* vm) {
    ...
    int type = vm->isTwoNode ? 1 : 0;
    double nowCap = (double)vm->core * vm->core + vm->memory * vm->memory;
    for(int i = 1; i < typesOfServer2Choose; ++i) {
        if(nowCap < coreMemCapSplit[type][i]) {
#ifdef TEST
            // cerr << "getRowOfvm2server: " << *vm << ": " << i-1 << "\n";
#endif
            return i - 1;
        }
    }
#ifdef TEST
    // cerr << "getRowOfvm2server: " << *vm << ": " << typesOfServer2Choose - 1 << "\n";
#endif
    return typesOfServer2Choose - 1;
}

//一系列点(x,y)，求最小二乘斜率
double getLSK(vector<pair<int, int>> points) {
    double numerator = 0;
    double denominator = 0;
    for(int i = 0; i < points.size(); ++i) {
        numerator += (double)points[i].first * points[i].second;
        denominator += (double)points[i].first * points[i].first;
    }
...
serverInfo* findProperServer(double ratio, int minCore, int minMem, int maxCore, int maxMem) {
    // i < (int)serverInfos.size()
    serverInfo* server = nullptr;
    serverInfo* candiate = nullptr;
    double x1 = 1, y1 = x1 * ratio;
    // double bestSimilar = (x1 * x_2 + y1 * y_2) * (x1 * x_2 + y1 * y_2) / ((x1 * x1 + y1 * y1) * (x_2 * x_2 + y_2 * y_2));
    double bestSimilar = 0;
    for(int i = 0; i < (int)serverInfos.size(); ++i) {
        double x2 = serverInfos[i].A.memory, y2 = serverInfos[i].A.core;
        double nowSimilar = (x1 * x2 + y1 * y2) * (x1 * x2 + y1 * y2) / ((x1 * x1 + y1 * y1) * (x2 * x2 + y2 * y2));
        if(x2 > minMem && y2 > minCore && x2 < maxMem + 100 && y2 < maxCore + 100) {
            if(candiate == nullptr) candiate = &serverInfos[i];
            else if(candiate->A.memory + candiate->A.core > serverInfos[i].A.memory + serverInfos[i].A.core) {
                candiate = &serverInfos[i];
            }
        }
        if(server == nullptr && x2 > minMem && y2 > minCore && x2 < maxMem && y2 < maxCore) {
            server = &serverInfos[i];
            bestSimilar = (x1 * x2 + y1 * y2) * (x1 * x2 + y1 * y2) / ((x1 * x1 + y1 * y1) * (x2 * x2 + y2 * y2));
        }
        if(nowSimilar > bestSimilar && x2 > minMem && y2 > minCore && x2 < maxMem && y2 < maxCore) {
            bestSimilar = nowSimilar;
            server = &serverInfos[i];
        }
    }
    // assert(server != nullptr);
    if(server == nullptr) server = candiate;
    return server;
}
...
void chooseServers() {
    ...
    vector<pair<int, int>> doubleNodeVm;
    vector<pair<int, int>> singleNodeVm;
    for(const auto& oneDayReqs : allDaysRequests) {
        for(const auto& req : oneDayReqs) {
            if(req.type == ADD) {
                vmInfo* vm = vmInfos[req.vmName];
                if(vm->isTwoNode) {
                    doubleNodeVm.emplace_back(vm->memory, vm->core);
                } else {
                    singleNodeVm.emplace_back(vm->memory, vm->core);
                }
            }
        }
    }
    sort(singleNodeVm.begin(), singleNodeVm.end(), [&](const pii & a, const pii & b) {
        return (ll)a.first * a.first + (ll)a.second * a.second <
               (ll)b.first * b.first + (ll)b.second * b.second;
    });
    sort(doubleNodeVm.begin(), doubleNodeVm.end(), [&](const pii & a, const pii & b) {
        return (ll)a.first * a.first + (ll)a.second * a.second <
               (ll)b.first * b.first + (ll)b.second * b.second;
    });

    int singleSz = singleNodeVm.size(), doubleSz = doubleNodeVm.size();
    int singleStep = singleSz / typesOfServer2Choose, doubleStep = doubleSz / typesOfServer2Choose;
    vector<int> singleSplitIdx, doubleSplitIdx;
    for(int i = 0; i < typesOfServer2Choose; ++i) {
        singleSplitIdx.emplace_back(i * singleStep);
        doubleSplitIdx.emplace_back(i * doubleStep);
        coreMemCapSplit[0][i] = (double)singleNodeVm[i * singleStep].first * singleNodeVm[i * singleStep].first +
                                singleNodeVm[i * singleStep].second * singleNodeVm[i * singleStep].second;
        coreMemCapSplit[1][i] = (double)doubleNodeVm[i * doubleStep].first * doubleNodeVm[i * doubleStep].first +
                                doubleNodeVm[i * doubleStep].second * doubleNodeVm[i * doubleStep].second;
    }
    singleSplitIdx.emplace_back(singleSz);
    doubleSplitIdx.emplace_back(doubleSz);
    coreMemCapSplit[0][typesOfServer2Choose] = (double)singleNodeVm[singleSz - 1].first * singleNodeVm[singleSz - 1].first +
            singleNodeVm[singleSz - 1].second * singleNodeVm[singleSz - 1].second;
    coreMemCapSplit[1][typesOfServer2Choose] = (double)doubleNodeVm[doubleSz - 1].first * doubleNodeVm[doubleSz - 1].first +
            doubleNodeVm[doubleSz - 1].second * doubleNodeVm[doubleSz - 1].second;
    //处理单节点
#ifdef TEST
    cerr << "single ratio: ";
#endif
    for(int split = 0; split < typesOfServer2Choose; ++split) {
        int mxCore = 0, mxMem = 0;
        for(int i = singleSplitIdx[split]; i < singleSplitIdx[split + 1]; ++i) {
            mxCore = max(singleNodeVm[i].second, mxCore);
            mxMem = max(singleNodeVm[i].first, mxMem);
        }
        double ratio = getLSK(vector<pii>(singleNodeVm.begin() + singleSplitIdx[split], singleNodeVm.begin() + singleSplitIdx[split + 1]));
#ifdef TEST
        cerr << ratio << " ";
#endif
        serverInfo* server = findProperServer(ratio, mxCore, mxMem, mxCore + 50, mxMem + 50);
        // serverInfo* server = findProperServer(ratio, mxCore + (split + 3) * 20, mxMem + (split + 3) * 20, mxCore + (split + 3) * 30, mxMem + (split + 3) * 30);
        chosenServers[0][split] = *server;
    }
    ...
    for(int split = 0; split < typesOfServer2Choose; ++split) {
        int mxCore = 0, mxMem = 0;
        for(int i = doubleSplitIdx[split]; i < doubleSplitIdx[split + 1]; ++i) {
            mxCore = max(doubleNodeVm[i].second, mxCore);
            mxMem = max(doubleNodeVm[i].first, mxMem);
        }
        double ratio = getLSK(vector<pii>(doubleNodeVm.begin() + doubleSplitIdx[split], doubleNodeVm.begin() + doubleSplitIdx[split + 1]));
#ifdef TEST
        cerr << ratio << " ";
#endif
        serverInfo* server = findProperServer(ratio, mxCore / 2, mxMem / 2, mxCore / 2 + 50, mxMem / 2 + 50);
        // serverInfo* server = findProperServer(ratio, mxCore / 2 + (split + 3) * 20, mxMem / 2 + (split + 3) * 20, mxCore / 2 + (split + 3) * 30, mxMem / 2 + (split + 3) * 30);
        chosenServers[1][split] = *server;
    }
#ifdef TEST
    cerr << "\n";
    for(int type = 0; type < 2; ++type) {
        for(int i = 0; i <= typesOfServer2Choose; ++i) {
            cerr << coreMemCapSplit[type][i] << ", ";
        }
        cerr << "\n";
    }
#endif
    ...
    //                 if(costEffect > nowCostEffect && serverInfos[i].A.core < MaxcoreMaxmemory[type][0] + (2 - type) * 50 && serverInfos[i].A.memory < MaxcoreMaxmemory[type][1] + (2 - type) * 80 &&
    //                         serverInfos[i].A.core >= MaxcoreMaxmemory[type][0] && serverInfos[i].A.memory >= MaxcoreMaxmemory[type][1]) {
    //                     nowSer = &serverInfos[i];
    //                 }
    //             }
    ...
    if(isTwoNodeOfVm == 0) { //单节点服务器的处理
        if((int)vmsPlacedNewFor1NodeServers[row].size() > maxVmSize) { //这一行的虚拟机数量大于一定量
            while((int)vmsPlacedNewFor1NodeServers[row].size() > minVmSize) {
                ...
                // }
                // // int randIdx = rand() % vmsPlacedNewFor1NodeServers[row].size();
                firstFitSuccessful(vmsPlacedNewFor1NodeServers[row].back());
                // vmsPlacedNewFor1NodeServers[row].erase(vmsPlacedNewFor1NodeServers[row].begin()+randIdx);
                vmsPlacedNewFor1NodeServers[row].pop_back();
            }
        ...
        if((int)vmsPlacedNewFor2NodeServers[row].size() > maxVmSize) { //这一行的虚拟机数量大于一定量
            while((int)vmsPlacedNewFor2NodeServers[row].size() > minVmSize) {
                ...
                // // int randIdx = rand() % vmsPlacedNewFor2NodeServers[row].size();
                firstFitSuccessful(vmsPlacedNewFor2NodeServers[row].back());
                // vmsPlacedNewFor2NodeServers[row].erase(vmsPlacedNewFor2NodeServers[row].begin()+randIdx);
                vmsPlacedNewFor2NodeServers[row].pop_back();
            }
            //将前面因为虚拟机还没有加而没有释放的资源释放掉
...
void migrateSingleNodeServersFirstFit_fast(int& maxSingleNodeServerMigrateTime, int& realMigrateTime, vector<intTriple>& migration) {
    ...
    int splitCnt = (ll)vmCnt * 900 / 1000;
    int nowCnt = 0;
...
//返回的下标及以后的服务器是待迁移的
int splitServerIdx(int splitVmNum, const vector<serverEntity*>& vecServer) {
    int i = vecServer.size() - 1;
    int vmNums = 0;
    for(; i >= 0; --i) {
        vmNums += vecServer[i]->vmCnt;
        if(vmNums >= splitVmNum)   return i;
    }
    return -1;
}

//迁移双节点服务器  BestFit,传入的splitVmNum是旧服务器待转移的虚拟机的数量
void migrateDoubleNodeServersBestFit(int splitVmNum, int& maxDoubleNodeServerMigrateTime, int& realMigrateTime, vector<intTriple>& migration) {
    realMigrateTime = 0;
    cerr << " splitVmNum  = " << splitVmNum << " ";
    //2.vector从前往后遍历，首先判断服务器上是否有虚拟机，如果没有虚拟机，就直接跳过如果有虚拟机，则返回所有的虚拟机实体。
    //3.从后往前遍历，根据虚拟机的core/memory选出对应的服务器，进行资源的配置，然后返回被分配的服务器编号，并且将原服务器的资源释放
    //4.判断返回的服务器编号，如果编号相同，则表示迁移失败，没有服务器可以进行迁移，或者迁移次数达到上限，则这组服务器的迁移完成，然后进行下一组服务器的迁移
    ...
    int splitSerIdx = splitServerIdx(splitVmNum, vecServer); //splitSerIdx及以后的服务器都是待迁移的，则做BestFIt时只能到下标  splitSerIdx-1
    cerr << " splitSerIdx = " << splitSerIdx << " " << "  vecServer.size = " << vecServer.size() << " ";
    int len = vecServer.size();
    for(int right = vecServer.size() - 1; right >= 0 && maxDoubleNodeServerMigrateTime > 0; --right) {
        // for (auto it_backward = serverFor2Node.rbegin(); it_backward != serverFor2Node.rend() && maxDoubleNodeServerMigrateTime > 0; ++it_backward) { //前面的虚拟机数量少的，往虚拟机数量多的服务器上放.每种服务器都要遍历一次
        //判断此服务器上是否有虚拟机
        ...
        for(vmEntity* vm : vmSetOfServer) {
            ...
            serverEntity* bestServer = NULL; //保存最好的server
            nodeType freeType;//空闲的节点
            for(left = 0; left < splitSerIdx; ++left) {//在[0,splitSerIdx-1]进行BestFit
                // for(; it_forward->first != it_backward->first; ++it_forward) { //服务器从前往后遍历
                // 找到一个对应类型的服务器,根据虚拟机判断当前服务器是否同一类型
                //返回虚拟机对应的服务器的下标
                // serverEntity* newServer = it_forward->second;
                ...
                // if(newServer->serverName == chosenServers[1][vmServerTypeIndex].serverName)
                {
                    ...
                    if(layPlace == 2) { //双节点部署，则进行服务器的迁移
                        ...
                        if(bestServer == NULL) {
                            ...
                            if(newServer->A.core < bestServer->A.core)
                                bestServer = newServer;
                        }
            ...
            if(bestServer != NULL) {
                ...
                maxDoubleNodeServerMigrateTime--;
                realMigrateTime++;
                vmsInServers[bestServer].insert(vm);
                vmsToBeDeleted.push_back(vm);//保存待转移的虚拟机迭代器
                vmRePlaced(vm, ser, bestServer, freeType);
                //migrate信息记录
                migration.emplace_back(vm->id, bestServer->serverId, 2); //
                cerr << " ;  migrate ;  ";
            }
        }
    ...
    cerr << " realMigrateTime = " << realMigrateTime << endl;
}
...
void migrate(vector<intTriple>& migration) {
    ...
    */
    ...
    if(serverFor2Node.size() > 2500) {
        ...
        //migrateDoubleNodeServersBestFit(migrationTimeTmp, doubleRealMigrateTime, doubleRealMigrateTime, migrationDoubleNode);
    }

    if(serverFor1Node.size() > 2500) {
        ...
        if(oneDayRequests[i].type == DEL) { //操作DEL
            ...
            if(!hasPlacedToOldServer) { //没有成功加到旧服务器里面
                ...
                if(vm->isTwoNode == 0) { //单节点虚拟机？？？？？这个tw判断一下
                    ...
                    proceessVmsPlacedNewServers(0, vmIdxBelongingToservers, MAXSIZE / 4, MAXSIZE / 3); //-2只是为了防止越界，这里可以减去其他数都行

                } else if(vm->isTwoNode == 1) {
                    vmsPlacedNewFor2NodeServers[vmIdxBelongingToservers].push_back(vm);//将vm放置到对应的里面去
                    proceessVmsPlacedNewServers(1, vmIdxBelongingToservers, MAXSIZE / 4, MAXSIZE / 3);
                }
            }
        }
    }
...
int main() {
    ...
    srand(time(NULL));
#ifdef TEST
    freopen("../training-data/training-1.txt", "rb", stdin);
    freopen("../training-data/training-1-output.txt", "w", stdout);
#endif
    cin >> N_allServerType;
#ifdef TEST
