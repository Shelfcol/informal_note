...
#define B nodes[1]
    ...
    void alloc(int A_core, int A_mem, int B_core, int B_mem) {
        ...
        this->B.memory -= B_mem;
        ++vmCnt;
        mxSource.insert(A_core+A_mem+B_core+B_mem);
    ...
    void deAlloc(int A_core, int A_mem, int B_core, int B_mem) {
        ...
        this->B.memory += B_mem;
        --vmCnt;
        mxSource.erase(mxSource.find(A_core + A_mem + B_core + B_mem));
    }

    int findMxVm() const {
        if(vmCnt==0) return 0;
        else return *mxSource.rbegin();
    }
