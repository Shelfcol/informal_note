...
volatile static int serverNumber = 0; //服务器数量（编号从0开始）
...
const float serverCoreMemoryRatio = 3.0;
const float vmCoreMemoryRatio = 3.0;
...
int buyServer(vmEntity* vm) { //根据虚拟机购买对应的服务器，需要考虑被core/memory和是否是双节点
...
void match(vmEntity* vm) {
    ...
    if (vm->isTwoNode) { //双节点
        ...
        //检查是否部署有问题
        if (serverFor2Node[best]->A.core < 0 || serverFor2Node[best]->A.memory < 0 || serverFor2Node[best]->B.core < 0 || serverFor2Node[best]->B.memory < 0) {
            cout << "twoNode failure............" << endl;
            exit(-1);
        }
        ...
        //assert(bestSer != -1);
        //if (bestSer != -1)
        //    cout << "match bestSer  num =" << bestSer << endl;
        /*****************************************************************************/
        if (bestSer == -1) { //表示没找到合适的服务器，则需要根据虚拟机的core/memory来购买对应的服务器
            bestSer = buyServer(vm);
        ...
        if (bestNode == 0) {
            ...
            /*if (vm->id == 487389203)
            {
                //cout <<  " 487389203  debug core =" << core << " memory =" << memory << ";serverA core=" << serverFor1Node[bestSer]->A.core << " memory= " << serverFor1Node[bestSer]->A.memory << endl;
            }*/
            serverFor1Node[bestSer]->A.core -= core;
            serverFor1Node[bestSer]->A.memory -= memory;
            vmArrangeToday.emplace_back("(" + to_string(bestSer) + ", A)\n");
            if (serverFor1Node[bestSer]->A.core < 0 || serverFor1Node[bestSer]->A.memory < 0) {
                cout << "A failure............" << endl;
                exit(-1);
            }
        } else {
            ...
            if (serverFor1Node[bestSer]->B.core < 0 || serverFor1Node[bestSer]->B.memory < 0) {
                cout << "B failure............" << endl;
                exit(-1);
            }
        }
...
void delVm(int id) {
    ...
    if (vm->isTwoNode) {
        ...
        //cout << "deleted server->A.core =" << server->A.core << endl;
        //释放资源
...
void dailyProcess(int day) {
    ...
    //回收资源
    //cout << "start recycle resource.." << endl;
    ...
    for (const requestInfo& req : dailyRequests) {
        if (req.type == ADD) {
            vmEntity* vm = (vmInfos[req.vmName])->clone(); //新建一个虚拟机
            vm->id = req.vmId;
