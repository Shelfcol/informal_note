...
double ratio_core_mem = 0.5; //core和memory之间的重要性关系
pair<int, int> match(vmEntity* vm) {
    ...
    if (vm->isTwoNode) { //双节点
        ...
        result.second = 2;
        //分配资源
        serverFor2NodeBoughtToday[best]->alloc(core, memory, core, memory);
...
pair<int, int> addVm(vmEntity* vm) {
    //添加虚拟机
    pair<int, int> result = match(vm); //分配给服务器，如果对应的服务器的资源不够，则购买对应的服务器资源
...
void delVm(int id) {
    ...
    if (vm->isTwoNode) {
        ...
        if (iter != serverFor2Node.end()) {
            ...
            //释放资源
            server->deAlloc(vm->core / 2, vm->memory / 2, vm->core / 2, vm->memory / 2);
...
void dailyProcess(int day) {
        ...
        if(reqIdx[i].second->type == DEL) {
            if(i > last) {
                sort(reqIdx.begin() + last, reqIdx.begin() + i, [&](const piReq & a, const piReq & b) {
                    return vmInfos[a.second->vmName]->core + vmInfos[a.second->vmName]->memory
                           > vmInfos[b.second->vmName]->core + vmInfos[b.second->vmName]->memory;
                });
            }
            last = i + 1;
        }
    ...
    for(auto [ser, node] : outp) {
        if(ser != -1) {
            if(node == 2) {
                outputVmInfosToday.emplace_back(0, ser, ser, "D");
            } else {
                if(node == 0) outputVmInfosToday.emplace_back(0, ser, ser, "A");
                else outputVmInfosToday.emplace_back(0, ser, ser, "B");
            }
        }
    }
