...
void runJudger(std::string inputFile, std::string outputFile) {
    ...
    for (int date = 0; date < T; ++date) {
        // if(date % 100==0) cout << "day: " << date << std::endl;
        auto add_vir_2_ser = [&](const std::string & name, int add_id, int pos, int numa = -1) {
            ...
            if(date==-1)
                cout << "ser " << pos << ": (" << sers[pos].A.first << ", " << sers[pos].A.second << "), (" 
                    << sers[pos].B.first << ", " << sers[pos].B.second << ")\n";
            if (sers[pos].A.first < 0 || sers[pos].A.second < 0 ||
                    sers[pos].B.first < 0 || sers[pos].B.second < 0) {
                if(date==-1) {
                    if(sers[pos].A.first < 0) std::cout << "A core not enough\n";
                    if(sers[pos].A.second < 0) std::cout << "A mem not enough\n";
                    if(sers[pos].B.first < 0) std::cout << "B core not enough\n";
                    if(sers[pos].B.second < 0) std::cout << "B mem not enough\n";
                }
        ...
        if (mi_num > 3 * virs.size() / 100) {
            ...
            std::cout << "date " << date << ": migration exceed 3*n/100\n";
            exit(0);
        }
        for (int i = 0; i < mi_num; ++i) {
            ...
            if (virs[vir_id].type == 0) {
                std::string A_or_B = tmp_str.substr(0, tmp_str.find(','));
...
int main(int argc, char* argv[]) {
    ...
    std::string input = "training-" + filename + ".txt";
    std::string output = "training-" + filename + "-output.txt";
    runJudger(input.c_str(), output.c_str());
    return 0;
}